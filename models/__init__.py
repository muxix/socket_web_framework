...
def save(data, path):
    ...
    # json.dumps: 将 python 对象编码成 json 字符串, encode
    s = json.dumps(data, indent=2, ensure_ascii=False, cls=JSONEncoderExtend)
...
def load(path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
        s = f.read()
        log('load', s)
        # json.loads: 将已编码的 json 字符串解码为 python 对象, decode
        return json.loads(s, object_hook=json_decode_extend)
